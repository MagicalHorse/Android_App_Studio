...
public abstract class Abs_HomeItemInfo {
     ...
     * 外部调用
     *****/
    public void setParetView(View view,Activity activity) {
    ...
    public void setBrand(List list)
    {
        if(brandlist_linearlayout!=null)
        {
            ...
            showDataCount=maxBrandCount;
            int screenWidth=ToolsUtil.getDisplayWidth(activity);
            int marginvalue = activity.getResources().getDimensionPixelOffset(R.dimen.branditem_margin);
    ...
    public void setRecommendProduct(List list,boolean b) {
        ...
        if (recommendproduct_linearlayout != null) {
            ...
            for (int i = 0; i < maxpicCount; i++) {
                View child = LayoutInflater.from(activity).inflate(R.layout.home_item_img_layout, null);
                ...
                int marginvalue=activity.getResources().getDimensionPixelOffset(R.dimen.item_margin);
                params.leftMargin=marginvalue;
                params.rightMargin=marginvalue;
    ...
    void setpicBitmap(View v,Bitmap bitmap)
    {
        if(v instanceof  ImageView)
        {
            if(bitmap==null)
            {
                ((ImageView) v).setImageResource(R.drawable.default_pic);
            }else
            {
