...
public class ApplyForRefundActivity extends BaseActivityWithTopView implements OnClickListener {
    ...
    RequestBaiJiaOrdeDetailsInfoBean requestBaiJiaOrdeDetailsInfoBean;//订单信息
    BaiJiaOrdeDetailsInfoBean baiJiaOrderListInfo;
    MyListView applyforrefund_layout_product_mylistview;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (this.getIntent().getStringExtra("OrderNo") == null) {
            MyApplication.getInstance().showMessage(ApplyForRefundActivity.this, "订单id 错误，请重试");
            finish();
            return;
        } else {
    ...
    void initView() {
        ...
        applyforrefund_layout_product_mylistview = (MyListView) parentView.findViewById(R.id.applyforrefund_layout_product_mylistview);
    ...
    {
        obj_list.clear();
        if(requestBaiJiaOrdeDetailsInfoBean!=null && requestBaiJiaOrdeDetailsInfoBean.getData()!=null )
        {
            baiJiaOrderListInfo=requestBaiJiaOrdeDetailsInfoBean.getData();
            obj_list.addAll(requestBaiJiaOrdeDetailsInfoBean.getData().getProduct());
        }
        baijiaOrderDetailsAdapter=new BaijiaOrderDetailsAdapter(ApplyForRefundActivity.this,obj_list);
        applyforrefund_layout_product_mylistview.setAdapter(baijiaOrderDetailsAdapter);
        baijiaOrderDetailsAdapter.notifyDataSetChanged();

        if(requestBaiJiaOrdeDetailsInfoBean.getData().getProduct()!=null && requestBaiJiaOrdeDetailsInfoBean.getData().getProduct().size()>0)
        {
            createorder_dialog_layout_countvalue_edittext.setText(Integer.toString(baiJiaOrderListInfo.getProduct().size()));
        }

        applyforrefund_layout_refundprivevalue_textview.setText("￥" + ToolsUtil.DounbleToString_2(baiJiaOrderListInfo.getActualAmount()));
        ...
        httpControl.getBaijiaOrderDetails(OrderNo, true, new HttpCallBackInterface() {
            ...
            public void http_Success(Object obj) {
                if (obj != null && obj instanceof RequestBaiJiaOrdeDetailsInfoBean) {
                    requestBaiJiaOrdeDetailsInfoBean = (RequestBaiJiaOrdeDetailsInfoBean) obj;
